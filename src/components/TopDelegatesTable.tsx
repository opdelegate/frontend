...
export const TopDelegatesTable = ({
  ...
  const [connectedUserDelegate, setConnectedUserDelegate] =
    useState<Delegates | null>(null);
  const [searchValue, setSearchValue] = useState("");
  const [searchedResult, setSearchedResult] = useState<Delegates[]>([]);

  const setUserData = useCallback(() => {
    if (connectWalletAddress) {
      const connectedUserDelegate = data.find(
        (d) => d.address === connectWalletAddress
      );
  ...
  useEffect(() => {
    const filteredData = data.filter((d) => {
      return (
        d.address.toLowerCase().includes(searchValue.toLowerCase()) ||
        (d.ensName &&
          d.ensName.toLowerCase().includes(searchValue.toLowerCase()))
      );
    });
              ...
              <Tbody w="100%">
                <Tr w="100%" backgroundColor="#F30F210D">
                  ...
                  <Td
                    ...
                    fontWeight={700}
                  >
                    {connectedUserDelegate.ensName !== null
                      ? connectedUserDelegate.ensName
                      : formatAddress(connectedUserDelegate.address)}
                  </Td>
                  <Td
                    w="30%"
                    borderColor="#FF0420"
            ...
            <Tbody w="100%">
              {dataToShow.map((d, index) => (
                <Tr
                  ...
                  w="100%"
                  backgroundColor={
                    d.address === connectedUserDelegate?.address
                      ? "#F30F210D"
                      : index % 2 === 1
                      ? "#EBF2FF80"
                      : "inherit"
